...
# Calculate expected reward
mean_counts_mpcreg, std_counts_mpcreg = calculate_expected_reward(all_s, all_u)

# plot!(time_steps, [mean(all_mp_estimates[plotting_seed][i]) for i in 1:t], ribbon=[std(all_mp_estimates[plotting_seed][i]) for i in 1:t], label="BiLQR")
