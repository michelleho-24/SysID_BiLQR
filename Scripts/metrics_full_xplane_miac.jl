...
@load "bilqr_xplanefull_miac_results.jld2" all_b_ends all_AB_estimates all_AB_variances all_ΣΘΘ all_s all_u all_ABtrue 
...
last_log_probs_bilqr = [log_multivariate_normal_pdf(all_ABtrue[seed], all_AB_estimates[seed][end], all_AB_variances[seed][end]) for seed in 1:length(all_AB_estimates)  if haskey(all_AB_estimates, seed)]
...
@load "random_xplanefull_miac_results.jld2" all_b_ends all_AB_estimates all_AB_variances all_ΣΘΘ all_s all_u all_ABtrue 
...
@load "mpc_xplanefull_miac_results.jld2" all_b_ends all_AB_estimates all_AB_variances all_ΣΘΘ all_s all_u all_ABtrue 
...
last_log_probs_mpc = [log_multivariate_normal_pdf(all_ABtrue[seed], all_AB_estimates[seed][end], all_AB_variances[seed][end]) for seed in 1:length(all_AB_estimates)  if haskey(all_AB_estimates, seed)]
# filtered_last_log_probs_random = last_log_probs_random[non_negative_indices]
# Calculate the average and standard deviation
...
# log_probs_mpcreg = [log_multivariate_normal_pdf(all_ABtrue[plotting_seed ], all_AB_estimates[plotting_seed ][i], all_AB_variances[plotting_seed][i]) for i in 1:t]
...
println("MPC Reg: ", trace_mpc_avg, " ± ", trace_mpc_ste)
...
println("MPC Reg: ", avg_last_log_prob_mpcreg, " ± ", std_last_log_prob_mpcreg)
...
println("MPC Reg: ", avg_reward_mpcreg, " ± ", std_reward_mpcreg)
