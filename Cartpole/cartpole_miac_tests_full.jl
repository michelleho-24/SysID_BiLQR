...
function system_identification(method)
    ...
    Σ0 = Diagonal(vcat(fill(1e-10, num_states(pomdp) - num_sysvars(pomdp)), [pomdp.Σ0[]]))  # Initial covariance
    
    # initialize belief as true state vector, mean of the log of pole mass, and new Σ0
    b = vec(vcat(pomdp.μ[1:num_states(pomdp)-num_sysvars(pomdp)], s_true[5], Σ0[:]))
    ...
    return all_b, mp_estimates, mp_variances, ΣΘΘ, all_s
