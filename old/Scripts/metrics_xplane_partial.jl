...
# function log_prob_multivariate_gaussian(x::Vector, mean::Vector, covariance::Matrix)
#     # Ensure dimensions match
#     k = length(x)
#     @assert k == length(mean) "Mean vector and data point must have the same length"
#     @assert size(covariance) == (k, k) "Covariance matrix must be square and match the dimensionality of the data"

#     # Calculate the log determinant and the inverse of the covariance matrix
#     log_det_cov = logdet(covariance)
#     inv_cov = inv(covariance)

#     # Compute the Mahalanobis distance
#     diff = x - mean
#     mahalanobis_distance = dot(diff, inv_cov * diff)

#     # Compute the log PDF
#     return -0.5 * (k * log(2 * π) + log_det_cov + mahalanobis_distance)
# end
...
@load "bilqr_xplanepartial_sysid_results.jld2" all_b_ends all_AB_estimates all_AB_variances all_ΣΘΘ all_s all_u all_ABtrue 
...
@load "random_xplanepartial_sysid_results.jld2" all_b_ends all_AB_estimates all_AB_variances all_ΣΘΘ all_s all_u all_ABtrue 
