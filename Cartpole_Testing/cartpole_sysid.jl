...
@with_kw mutable struct CartpoleMDP <: iLQGPOMDP{AbstractVector,AbstractVector,AbstractVector}
    ...
    # Σ0::Matrix{Float64} = Diagonal([0.5, 0.5, 0.5, 0.5, 1.0])  # Initial belief covariance
    # b0::Vector{Float64} = vcat(μ0, Float64.(Σ0[:]))  # Initial belief vector

    # s_goal::Vector{Float64} = [0.0, π/2, 0.0, 0.0, 2.0, vec(zeros(5, 5))...]
    ...
    W_obs::Matrix{Float64} = 1e-4 * Matrix{Float64}(I, 4, 4)
end
